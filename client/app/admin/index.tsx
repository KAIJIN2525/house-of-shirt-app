...
export default function AdminScreen() {
  ...
  const readyExports = exportHistory.filter((record) => record.status === "Ready");
  ...
  const recentSellableOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      return !status.includes("cancel") && !status.includes("refund") && !Number.isNaN(placedAt) && placedAt >= cutoff;
    });
  }, [orders]);
  ...
  const bestSellingSizes = useMemo(() => {
    ...
    recentSellableOrders.forEach((order) => order.lineItems?.forEach((item) => {
      ...
      if (foundSize) counts[foundSize] = (counts[foundSize] || 0) + item.quantity;
    }));
  ...
  const variantSales = useMemo(() => {
    ...
    recentSellableOrders.forEach((order) => order.lineItems?.forEach((item) => {
      ...
      products.forEach((product) => product.sizeOptions?.forEach((option) => {
        if (product.name.trim().toLowerCase() === productName && variant.includes(option.value.toUpperCase())) {
  ...
  const lowStockAlerts = useMemo(() => {
    const alerts: { title: string; subtitle: string; level: "Critical" | "Low"; score: number }[] = [];
    products.forEach((product) => product.sizeOptions?.forEach((option) => {
      if (option.inventoryQuantity < 0 || option.inventoryQuantity > 3) return;
      const sold = variantSales.get(`${product.id}:${option.value}`) ?? 0;
      const daysLeft = sold > 0 ? Math.max(0, Math.round(option.inventoryQuantity / (sold / 30))) : null;
      alerts.push({
        title: `Size ${option.value} (${product.name})`,
        subtitle: option.inventoryQuantity === 0
          ? `Out · ${sold} sold / 30d`
          : `${option.inventoryQuantity} left${daysLeft !== null ? ` · ~${daysLeft}d` : ""}`,
        level: option.inventoryQuantity <= 1 ? "Critical" : "Low",
        score: (option.inventoryQuantity === 0 ? 1000 : 100 - option.inventoryQuantity * 10) + sold,
      });
    }));
    ...
    const fetchRestockCount = async () => {
      try {
        ...
        if (!error && count !== null) {
          ...
          const top = [...grouped.entries()].sort((left, right) => right[1] - left[1])[0];
  ...
  const salesChartData = useMemo(() => {
    ...
    if (salesView === "week") {
      ...
      return days.map(({ label, date }) => {
        const matching = orders.filter((order) => {
          ...
          return !status.includes("cancel") && !status.includes("refund") && !Number.isNaN(orderDate.getTime()) && orderDate.toDateString() === date.toDateString();
        });
        return {
          label,
    ...
    return months.map(({ label, year, monthIndex }) => {
      const matching = orders.filter((order) => {
        const status = order.status.toLowerCase();
        const orderDate = new Date(order.placedOn);
        return !status.includes("cancel") && !status.includes("refund") && !Number.isNaN(orderDate.getTime()) && orderDate.getFullYear() === year && orderDate.getMonth() === monthIndex;
      });
      return {
        label,
        value: matching.reduce((sum, order) => sum + order.total, 0),
  ...
  const previousSalesTotal = useMemo(() => {
    const now = new Date();
    const currentStart = salesView === "week"
      ? new Date(now.getFullYear(), now.getMonth(), now.getDate() - 6)
      : new Date(now.getFullYear(), now.getMonth() - 7, 1);
    const periodDays = salesView === "week" ? 7 : Math.max(1, Math.ceil((now.getTime() - currentStart.getTime()) / 86400000));
    const previousStart = new Date(currentStart);
    previousStart.setDate(previousStart.getDate() - periodDays);
    return orders.reduce((sum, order) => {
      const status = order.status.toLowerCase();
      const placedAt = new Date(order.placedOn).getTime();
      if (status.includes("cancel") || status.includes("refund") || Number.isNaN(placedAt)) return sum;
      return placedAt >= previousStart.getTime() && placedAt < currentStart.getTime() ? sum + order.total : sum;
    }, 0);
  }, [orders, salesView]);
  ...
  const formatNaira = (value: number) => `NGN ${Math.round(value).toLocaleString("en-NG")}`;
  return (
      ...
      >
        <View className="relative flex-row items-center justify-between px-6 pb-4 pt-4">
          <Pressable onPress={() => router.back()}>
            <Ionicons name="close" size={18} color={isDark ? "#f8fafc" : "#111111"} />
          </Pressable>
          ...
          <View className="flex-row items-center gap-4">
            <Pressable onPress={() => router.push("/admin/inbox" as any)} className="relative">
              <Ionicons name="notifications-outline" size={20} color={isDark ? "#f8fafc" : "#111111"} />
              {unreadAdminAlerts > 0 ? <View className="absolute -right-2 -top-2 min-w-[16px] rounded-full bg-red-500 px-1"><Text className="text-center text-[8px] font-bold text-white">{Math.min(unreadAdminAlerts, 99)}</Text></View> : null}
            </Pressable>
            <Pressable onPress={() => router.push("/admin/settings" as any)}>
              <Ionicons name="settings-outline" size={18} color={isDark ? "#f8fafc" : "#111111"} />
            </Pressable>
          </View>
        </View>
          ...
          <View className="gap-4">
            <View className="bg-white px-5 py-6 dark:bg-[#101215]">
              ...
              <View className="mt-4 flex-row items-center gap-2">
                <Text className={`font-bold text-[11px] ${revenueSummary.change !== null && revenueSummary.change < 0 ? "text-red-500" : "text-[#4aa972]"}`}>
                  {revenueSummary.change === null ? "—" : `${revenueSummary.change >= 0 ? "+" : ""}${revenueSummary.change.toFixed(1)}%`}
                </Text>
                <Text className="font-normal text-[11px] text-neutral-400">vs last month</Text>
              </View>
            </View>
            ...
            <View className="bg-[#1f2736] px-5 py-6">
              ...
              <View className="mt-4 flex-row items-center gap-2">
                ...
                <Text className="font-normal text-[11px] text-white/60">
                  {readyExports.length > 0 ? "Latest manifest ready for download" : "No completed exports"}
                </Text>
              </View>
            </View>
          </View>
            ...
            <View className="gap-5">
              ...
              <View>
                ...
                <View className="flex-row gap-2">
                  {bestSellingSizes.length === 0 ? <Text className="text-[11px] text-neutral-400">No size sales recorded in the last 30 days.</Text> : bestSellingSizes.map((item, idx) => (
                    <View
                      key={item.size}
              ...
              >
                <View>
                  ...
                  <Text className="mt-1 text-[22px] font-bold text-black dark:text-white">
                    {String(pendingRestocksCount).padStart(2, "0")} Pending Requests
                  </Text>
                  {topRestockDemand ? <Text preserveCase className="mt-1 max-w-[260px] text-[10px] text-neutral-400" numberOfLines={1}>{topRestockDemand}</Text> : null}
                </View>
        ...
        <View className="mt-8 px-6">
          {blacklistedOrderAlerts.length > 0 ? (
            ...
            >
              ...
              <View className="flex-1">
                <Text className="text-[9px] font-bold tracking-[1.3px] text-red-600 dark:text-red-400">BLACKLIST RISK</Text>
                <Text className="mt-1 text-[14px] font-bold text-black dark:text-white">
                  {blacklistedOrderAlerts.length} restricted {blacklistedOrderAlerts.length === 1 ? "order" : "orders"} need review
                </Text>
              </View>
          ...
          <AdminArrivalFollowUpCard
            ...
            onMarkContacted={(order) => void acknowledgeArrivalAlert(
              order.id,
              `Admin marked follow-up contacted for ${order.status.toLowerCase()} order via ${order.carrier}.`,
            )}
            onResolve={(order) => void resolveArrivalAlert(
              order.id,
