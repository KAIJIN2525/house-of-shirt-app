  ...
  persist(
    (set, get) => ({
      ...
      createOrder: async (input) => {
        const orderId = `HS-${Math.floor(10000 + Math.random() * 89999)}`;
        const now = new Date();
        const isNigeriaOrder = isNigerianState(input.shippingState);
        const placedOn = now.toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
          year: "numeric",
        });
        const isLagos = isLagosLocation(
          input.shippingState,
        ...
        if (isLagos) {
          ...
          if (hour < 16) {
            estimatedArrival = "Same Day Delivery";
          } else {
            const tomorrow = new Date(now);
            tomorrow.setDate(now.getDate() + 1);
            estimatedArrival = tomorrow.toLocaleDateString("en-US", {
              month: "short",
              day: "numeric",
            });
          }
        ...
        const order: OrderRecord = {
          ...
          customerRiskStatus: matchedBlacklistedCustomer
            ? "Blacklisted"
            : "None",
          customerRiskReason: matchedBlacklistedCustomer?.blacklistReason ?? "",
          placedOn,
          estimatedArrival,
          carrier: logistics.carrier,
          trackingNumber: `${logistics.trackingPrefix}${Math.floor(
          ...
          deliveryRegion: isNigeriaOrder ? "Nigeria" : "International",
          paymentMethod: input.paymentMethod,
          fulfillmentMethod: logistics.fulfillmentMethod,
          ...
          outreachNotes: matchedBlacklistedCustomer
            ? [
                {
                  id: `risk-${Date.now()}`,
                  text: `Blacklist alert: ${matchedBlacklistedCustomer.name} placed a new order. Reason on file: ${matchedBlacklistedCustomer.blacklistReason ?? "Restricted customer review required."}`,
          ...
          timeline: [
            ...
            },
            {
              ...
              title: "Shipped",
              description: isLagos
                ? "Prepared for House Rider dispatch"
                : isNigeriaOrder
                  ? "Prepared for GIG Logistics handoff"
                  : "Tailoring queue reserved for dispatch",
              date: "",
              active: false,
            },
            {
              id: "3",
              title: "In Transit",
              description: "Awaiting carrier handoff",
              date: "",
              active: false,
            },
            {
        ...
        if (matchedBlacklistedCustomer) {
          const { sendLocalNotification } = (
            await import("./notificationStore")
          ).useNotificationStore.getState();
          sendLocalNotification(
            "⚠️ BLACKLIST ALERT",
            `Restricted customer ${input.customerName} just placed an order (${orderId}). review recommended.`,
            { orderId },
          );
        }
          ...
          if (user) {
            const { error: insertError } = await supabase
              ...
              .insert({
                ...
                status: "Processing",
                total: input.total, // Required by original schema
                total_amount: input.total, // Required by aligned schema
            ...
            const { data: blacklistAlertResult, error: blacklistAlertError } = await supabase.functions.invoke(
              "blacklist-order-alert",
              {
                body: {
                  source: "app_order",
                  orderId,
            ...
            if (blacklistAlertError) {
              console.error("Blacklist admin alert error:", blacklistAlertError);
            }

            const serverRiskReason =
              typeof blacklistAlertResult?.reason === "string"
                ? blacklistAlertResult.reason
                : "";

            if (serverRiskReason) {
              order.customerRiskStatus = "Blacklisted";
              order.customerRiskReason = serverRiskReason;
            ...
            // Mirror order to Shopify Admin so it appears in the dashboard
            try {
              ...
              const fnRes = await fetch(
                `${supabaseUrl}/functions/v1/create-shopify-order`,
                {
                  method: "POST",
                  headers: {
              ...
              if (!fnRes.ok) {
                console.error(`Shopify mirror HTTP ${fnRes.status}:`, fnBody);
              } else {
                const result = JSON.parse(fnBody);
                console.log(`Shopify order created: ${result.shopifyOrderName} (#${result.shopifyOrderNumber})`);
                // Store Shopify IDs so cancellation webhooks can match this record later
              ...
              // Non-fatal — app order is already saved; Shopify sync failure should not block the user
              console.error("Shopify order mirror exception:", shopifyErr);
            }
            ...
            await supabase.from("app_notifications").insert({
              ...
              message: `Your order #${orderId} has been received and is now processing.`,
              label: "ORDER PLACED",
            ...
            await sendLocalNotification(
              "Order Confirmed! 🎉",
              `Order #${orderId} is being processed. Thank you for shopping with House of Shirts.`,
            );

            // Send confirmation email notification logic (legacy)
            const {
              sendOrderConfirmationNotification,
              cancelAbandonedCartNotification,
            } = await import("@/lib/notifications");
            await sendOrderConfirmationNotification(isLagos, `#${orderId}`);
            await cancelAbandonedCartNotification();
          }
        } catch (err) {
          console.error("Error saving order or notification:", err);
        }

        set((state) => ({
          orders: [order, ...state.orders],
          blacklistedOrderAlerts: order.customerRiskStatus === "Blacklisted"
            ? [order, ...state.blacklistedOrderAlerts]
            : state.blacklistedOrderAlerts,
        }));
        return order;
      },
